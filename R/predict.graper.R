...
#' @name predict.graper
#' @description Function to predict the response on a new data set using a fitted graper model.
...
predict.graper <- function(object, newX, type = c("inRange","response", "link"), ...){

  # sanity check
  if(ncol(newX) != nrow(object$EW_beta)) {
    stop("Number of columns in newX need to agree with number of predictors in the graper object.")
  }
  # sanity check
  if(!is(object, "graper")) {
