...
plotPosterior <- function(result, beta0 = NULL, gamma0 = NULL, tau0 = NULL, whichParam = c("tau", "beta", "gamma")) {
    ...
    if ("beta" %in% whichParam) {
        ...
        if (!is.null(dim(result$Sigma_beta))) {
            if (ncol(result$Sigma_beta) == 1 | nrow(result$Sigma_beta) == 1)
                result$Sigma_beta <- as.numeric(result$Sigma_beta)
        }
        if (is.null(dim(result$Sigma_beta)))
            result$Sigma_beta <- diag(result$Sigma_beta)  #transfrom to diagnoal matrix in fully fatorized case
...
plotVBFit <- function(result, whichParam = c("ELB", "tau", "beta", "gamma")) {
    ...
    if ("ELB" %in% whichParam)
        gg_ELB <- ggplot(result$trace_param, aes(x = iter, y = ELB)) + geom_line() + ggtitle("Evidence lower bound")
    if ("tau" %in% whichParam)
        gg_tau <- ggplot(result$trace_param, aes(x = iter, y = tau)) + geom_line() + ggtitle("Noise precision")
    if ("ELB" %in% whichParam & "ELB" %in% whichParam)
        gridExtra::grid.arrange(gg_ELB, gg_tau, nrow = 2) else if ("ELB" %in% whichParam)
        print(gg_ELB) else if ("tau" %in% whichParam)
        print(gg_tau)

    if ("beta" %in% whichParam) {
        tracebeta <- reshape2::melt(result$trace_param, id.vars = "iter", measure.vars = colnames(result$trace_param)[grep("beta", colnames(result$trace_param))])
        gg_beta <- ggplot(tracebeta, aes(x = iter, y = value)) + geom_line() + facet_wrap(~variable) + ggtitle("Linear model coefficients")
        print(gg_beta)
    }

    if ("gamma" %in% whichParam) {
        tracegamma <- reshape2::melt(result$trace_param, id.vars = "iter", measure.vars = colnames(result$trace_param)[grep("gamma", colnames(result$trace_param))])
