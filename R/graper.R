...
#'  For each group a different strength of penalization is determined adaptively.
...
#'  to fully factorize across features (faster, default). If FALSE, a
...
#' a matrix of predictors (\code{X}), a response vector (\code{y}) and
#' a vector of group memberships for each predictor in \code{X} (\code{annot}).
#' For each feature group as specified in \code{annot} a penalty factor
#' and sparsity level is learnt.
#'
...
#'
#'  As the optimization is non-convex is can be helpful to use multiple random initializations
#'  by setting \code{n_rep} to a value larger 1. The returned model is then chosen
#'  as the optimal fit with respect to the evidence lower bound (ELB).
#'
#'  Depending on the response vector a linear regression model (\code{family = "gaussian"})
#'  or a logistic regression model (\code{family = "binomial"}) is fitted.
...
#' \item{sigma2_tildebeta_s1, EW_tildebeta_s1, alpha_gamma, alpha_tau, beta_tau, Sigma_beta, alpha_pi, beta_pi}
      ...
      if (family == "gaussian") {
        ...
        res
          ...
          res
            ...
            stop("Family not implemented. Needs to be either binomial or gaussian.")
          }
})
